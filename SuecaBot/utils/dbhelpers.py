...
async def dealing(id: str, player: str, cards=cards):
    ...
    if player == players[0]:
        ...
        async with aiosqlite.connect("SuecaBot/database/games.db") as db:
            async with db.cursor() as cursor:
                for i in range(4):
                    await cursor.execute(f"UPDATE {id} SET cards = ?, trump_card = ? WHERE userid = ?", (' '.join(map(str,cards[i])), dealing_cards[0], players[i]))
            await db.commit()
        return "Done"
    elif len(players) != 4:
        return "NotEnough"
